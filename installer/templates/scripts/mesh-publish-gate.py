...
Behavior (per Cycle 2 / Agent E §A — deny-list pipeline):
  ...
  2. If `published: true`, load 3-tier deny-list:
       - NDA-[REDACTED-NDA] KEYWORDS  (imported from ~/.claude/bin/nda-[REDACTED-NDA]-gate.py)
...
NDA_GATE_PATH = Path.home() / ".claude" / "bin" / "nda-[REDACTED-NDA]-gate.py"
...
def load_nda_keywords() -> list[str]:
    """Import KEYWORDS from nda-[REDACTED-NDA]-gate.py to keep tiers in sync.
    Per Agent E: adding a term to nda-[REDACTED-NDA]-gate updates both gates atomically.
    """
    if not NDA_GATE_PATH.exists():
        return []
    try:
        spec = importlib.util.spec_from_file_location("nda_[REDACTED-NDA]_gate", NDA_GATE_PATH)
