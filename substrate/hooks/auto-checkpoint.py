...
def main():
    try:
        ...
        # Never crash the hook — but LOG the error so we can diagnose stale chains
        try:
            log_file = CHAIN_DIR / "auto-checkpoint.log"
            from datetime import datetime, timezone
            ts = datetime.now(timezone.utc).isoformat()
            with open(log_file, "a") as f:
                f.write(f"[{ts}] ERROR: {type(e).__name__}: {e}\n")
        except Exception:
            pass  # Truly last resort — if logging fails, give up silently
