...
## ⇒ Rule
- flag `~/.claude/state/afk-mode.flag` ON ⇒ ∀ response ends with `AFK:` menu, 10 items, most-probable first
...
- toggle: "afk on" / "afk off" (hook-handled, works from phone)

## 📦 Stack (full layer, not a one-off)
- **Hook**: `~/.claude/hooks/afk-mode-gate.py` (UserPromptSubmit) — menu enforcement + number interpretation + selection telemetry. Registered settings.json.
- **Signature corpus**: `memory/_system/afk_signatures/<user>.json` — per-user probabilistic reply classes derived from REAL history (will.json seeded from 2026-04..06 directives: approve_and_continue 0.18, approve_plus_scope_escalation 0.14, adjust_one_axis 0.12, formalize_or_hookify 0.10, push_public, honesty_probe, absorb_or_remember, status_pivot, draft_for_review, strategic_inference, hold_or_defer)
