...
**Gamified vibe coding** — the original game-AI loop, on an LLM. Operate the agent by picking numbered branches like a pick-your-own-adventure, except the branches are generated live and the choices do real work. See [STORY-MODE.md](STORY-MODE.md). For the web app (no hooks), there's a hookless drop-in: [STORY-MODE-LITE.md](STORY-MODE-LITE.md).
